...
func FullJitter(duration, min, max time.Duration, rng *rand.Rand) time.Duration {
	...
	return boundedDuration(time.Duration(rng.Int64N(int64(normalizedDur)))+min, min, max)
}
...
func (b *exponentialDecorrelatedJitter) Delay() time.Duration {
	...
	b.lastDelay = boundedDuration(time.Duration(b.rng.Int64N(nextMax-int64(b.min)))+b.min, b.min, b.max)
	return b.lastDelay
}
